...
@router.post("/register-user", response_model=ProfileSchema, dependencies=[Depends(PermissionMiddleware(AllowAny))])
async def register_user(data: UserRegistrationSchema, db_session: AsyncSession = Depends(get_db_session)):
...
@router.get("/profile", response_model=ProfileSchema, dependencies=[Depends(PermissionMiddleware(IsAuthenticated))])
