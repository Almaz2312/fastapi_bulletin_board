...
class Category(Base):
    ...
    sub_categories: Mapped[Optional[List["SubCategory"]]] = relationship(  # type: ignore
...
class SubCategory(Base):
    ...
    category: Mapped["Category"] = relationship(  # type: ignore
        back_populates="sub_categories", lazy="joined"
    )
...
class Advertisement(Base):
    ...
    user: Mapped["User"] = relationship(  # type: ignore
        back_populates="advertisements", lazy="joined", passive_deletes=True
    )
    sub_category_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("sub_category.id", ondelete="SET NULL")
    )
    sub_category: Mapped[Optional["SubCategory"]] = relationship(  # type: ignore
    ...
    price: Mapped[Numeric] = mapped_column(
        Numeric(scale=2), CheckConstraint("price >= 0")
    )
    ...
    images: Mapped[Optional[List["AdImage"]]] = relationship(
        back_populates="advertisement", lazy="selectin"
    )
...
class AdImage(Base):
    ...
    advertisement: Mapped["Advertisement"] = relationship(
        back_populates="images", lazy="joined", passive_deletes=True
    )
